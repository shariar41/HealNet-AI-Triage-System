...
def main():
    # Load Mendeley file only to get the full official symptom-column list
    ...
    print("Detected Mendeley disease column:", disease_col)
    print("Total symptom columns found:", len(symptom_cols))

    # Load Symptom2Disease
    df = pd.read_csv(SYMPTOM_RAW_CSV)
    print("Original Symptom2Disease rows:", len(df))
    ...
    # Map disease label -> triage label
    ...
    print("Rows after disease mapping:", len(df))
    print("Mapped diseases:")
    ...
    for _, row in df.iterrows():
        ...
        # Convert free-text symptom description into the same binary feature space
        # as the Mendeley symptom columns
    ...
    print("Saved figure:", f"{FIGURE_DIR}/symptom2disease_class_distribution.png")
